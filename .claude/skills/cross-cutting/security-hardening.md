...
Prerequisite: read `specs/platform/security-rules.md` before using this skill. Auth-specific patterns live in `backend/jwt-auth.md`, `backend/oauth-provider.md`, `backend/gateway-security.md`.
...
|---|---|---|
| A01 Broken Access Control | Enforce RBAC at controller + domain layer | `backend/gateway-security.md` |
| A02 Cryptographic Failures | TLS in transit, AES-256 at rest, no plain secrets | this skill |
| A03 Injection | Parameterized queries, input sanitization | `backend/validation.md` |
...
| A06 Vulnerable Components | Dependency scanning in CI | `infra/ci-cd.md` |
| A07 Auth Failures | Lockout, MFA, secure session store | `backend/jwt-auth.md`, `backend/redis-session.md` |
| A08 Software/Data Integrity | Signed artifacts, SBOM | `infra/ci-cd.md` |
| A09 Logging Failures | Structured logs, no PII leak | `cross-cutting/observability-setup.md` |
...
- See `infra/secrets-management.md` for storage and rotation patterns
...
| Internal service-to-service | mTLS via service mesh (`infra/service-mesh.md`) |
...
- Validate at controller layer with `@Valid` (see `backend/validation.md`)
