...
4. **Consumer wiring** — `messaging/event-implementation.md`
5. **Idempotency** — `messaging/idempotent-consumer.md`
6. **Retry / DLQ** — `messaging/consumer-retry-dlq.md`
7. **Schema versioning** — branch on `schemaVersion` per `cross-cutting/api-versioning.md`
8. **Observability** — `cross-cutting/observability-setup.md` plus consumer-specific metrics below
9. **Tests** — `backend/testing-backend.md`, `testing/testcontainers.md` with Kafka
...
| Strategy | When | Skill |
|---|---|---|
| Natural (upsert by event-derived key) | Read model materialization | `messaging/idempotent-consumer.md` §natural |
| Idempotency table by `eventId` | Side-effect-producing handlers (email, webhook) | `messaging/idempotent-consumer.md` §idempotency-table |
| Optimistic concurrency on aggregate | Updates to versioned aggregates | `messaging/idempotent-consumer.md` §version-check |
...
See `messaging/consumer-retry-dlq.md` for the full container factory bean.
