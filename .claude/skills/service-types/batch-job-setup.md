...
3. **Scheduler** — `backend/scheduled-tasks.md`
4. **Concurrency lock** — see "Distributed Lock" below
5. **Cursor / checkpoint** — see "Restart Safety" below
6. **Transaction boundaries** — `database/transaction-boundary.md` (commit per chunk, not per job)
7. **Admin endpoint or CLI** — see "Operator Control" below
8. **Observability** — `cross-cutting/observability-setup.md` plus job-specific metrics
