...
```
/pipeline-tasks
/pipeline-tasks order-service
/pipeline-tasks --dry-run
```

- No argument: process all tasks across all services
- Service name: filter tasks for that service only
- `--dry-run`: show execution plan for both phases, do not execute
