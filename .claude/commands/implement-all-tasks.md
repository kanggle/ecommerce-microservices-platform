...
description: Analyze all ready tasks, determine optimal strategy, and implement them via subagents in dependency order
---
...
```
/implement-all-tasks
/implement-all-tasks order-service
/implement-all-tasks --dry-run
```
...
- Always use `isolation: "worktree"` when launching task agents — this prevents file conflicts and keeps main branch safe on failure
- Always merge worktree branches between rounds — next round must see prior changes
