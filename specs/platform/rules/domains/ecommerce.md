...
이커머스 도메인의 에러 코드는 [../../error-handling.md](../../error-handling.md)에 정의되어 있다. 이 파일은 **복사하지 않고 참조만** 한다.

주요 도메인 에러 (원본 문서의 섹션 참조):

- **Product**: `PRODUCT_NOT_FOUND`, `PRODUCT_NOT_AVAILABLE`, `VARIANT_NOT_FOUND`
- **Inventory**: `INSUFFICIENT_STOCK`, `STOCK_RESERVATION_EXPIRED`
- **Cart**: `CART_ITEM_NOT_FOUND`, `CART_EMPTY`
- **Order**: `ORDER_NOT_FOUND`, `ORDER_STATE_TRANSITION_INVALID`, `ORDER_ALREADY_CANCELLED`
- **Payment**: `PAYMENT_ALREADY_COMPLETED`, `PAYMENT_FAILED`, `PAYMENT_AUTH_EXPIRED`
- **Promotion**: `COUPON_NOT_FOUND`, `COUPON_EXPIRED`, `COUPON_ALREADY_USED`, `COUPON_NOT_APPLICABLE`
- **Shipping**: `SHIPMENT_NOT_FOUND`, `ADDRESS_INVALID`
- **Review**: `REVIEW_NOT_ALLOWED_WITHOUT_PURCHASE`

신규 에러 코드가 필요하면 [../../error-handling.md](../../error-handling.md)를 먼저 갱신한 뒤 이 문서에는 참조만 추가한다.
...
- [ ] 새 에러 코드가 [../../error-handling.md](../../error-handling.md)에 등록되어 있는가?
