...
3. 시스템이 입력값을 검증한다.
   ...
   - 비밀번호 최소 요구사항 확인
   - 이름 필수값 확인
4. auth-service가 계정을 생성한다.
5. auth-service가 `UserSignedUp` 이벤트를 발행한다.
...
2. auth-service가 refresh token의 유효성을 확인한다.
...
2. auth-service가 해당 세션의 refresh token을 폐기한다.
